...
export function filterProjects(query:string,category:string,featured:boolean){
  const q=query.trim().toLocaleLowerCase();
  return projects.filter(p=>(!featured||p.featured)&&(!category||p.categories.includes(category))&&(!q||`${p.name} ${p.fullName} ${p.city} ${p.english} ${p.categories.join(' ')}`.toLocaleLowerCase().includes(q)));
}
