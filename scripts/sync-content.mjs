...
for(let i=0;i<catalog.length;i++){
 ...
 const note=id===19?'画册 OCR 中项目规模的总面积与地上面积存在矛盾，该字段暂不用于展示，原文可查。':id===25?'城市字段中的“会议”为 OCR 串行内容，展示为广州市；原文保留。':'';
 const displayFacts={...facts};if(id===19)delete displayFacts['项目规模'];if(id===25)displayFacts['所在城市']='广州市';
 const sourceUrl=`/sources/project-${id}.md`;await fs.writeFile(path.join(app,'public',sourceUrl),md);
 projects.push({id,slug,name,fullName:clean(md.split('\n')[0].replace(/^#\s*\d+\.\s*/,'')),english,categories:category==='历史保护'&&id===14?[category,'商业办公']:[category],featured:selectedIndex>=0,homeOrder:homeIds.indexOf(id),city:displayFacts['所在城市']||'地点未记载',year:displayFacts['设计时间']||displayFacts['竣工时间']||'年份未记载',facts:displayFacts,rawFacts:facts,summary:section(md,'项目简介'),analysis,source:`文字/03_全部案例/${file}`,sourceUrl,extraSource,sourceYear:2023,note,cover,gallery,reference:media.find(m=>m.kind==='画册版面'),lab:id===29?'day':([10,27,16,20,14].includes(id)?'pixel':null)});
}
